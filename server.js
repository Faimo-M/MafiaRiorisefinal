...
app.post('/api/roles', requireAuth, requireRole(['chefe']), async (req, res) => {
...
app.put('/api/roles/:id', requireAuth, requireRole(['chefe']), async (req, res) => {
  const { name, weekly_target, monthly_target } = req.body;
...
app.delete('/api/roles/:id', requireAuth, requireRole(['chefe']), async (req, res) => {
...
app.post('/api/members', requireAuth, requireRole(['chefe','subchefe','user']), async (req, res) => {
  const { name, role_id } = req.body;
  const r = await query('INSERT INTO members (name,role_id) VALUES (?,?)', [name, role_id || null]);
  res.json({ id: r.insertId });
});
app.put('/api/members/:id', requireAuth, requireRole(['chefe','subchefe','user']), async (req, res) => {
  const { name, role_id } = req.body;
  await query('UPDATE members SET name=?, role_id=? WHERE id=?', [name, role_id || null, req.params.id]);
  res.json({ ok: true });
});
app.delete('/api/members/:id', requireAuth, requireRole(['chefe','subchefe','user']), async (req, res) => {
...
app.post('/api/contributions', requireAuth, requireRole(['chefe','subchefe','user']), async (req, res) => {
  const { member_id, amount, note } = req.body;
  const user_id = req.session.user.id;
  const r = await query('INSERT INTO contributions (member_id,user_id,amount,note) VALUES (?,?,?,?)', [member_id, user_id, amount, note || null]);
  await query('UPDATE members SET total_contribution = COALESCE(total_contribution,0) + ? WHERE id=?', [amount, member_id]);
  res.json({ id: r.insertId });
});

/* Dashboard */
app.get('/api/dashboard/targets', requireAuth, async (req, res) => {
